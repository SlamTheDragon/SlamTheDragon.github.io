            ...
            <div>
                <div className={style.i2 + ' ' + style.cardC}
                    onKeyDown={(e: React.KeyboardEvent<HTMLDivElement>) => {
                        if (e.key === "Enter") { window.location.href = `https://slamthedragon.me/status?id=${latestWip ?? 'notfound'}` }
                    }}
                    tabIndex={0}
                    onClick={() => ( window.location.href = `https://slamthedragon.me/status?id=${latestWip ?? 'notfound'}`)}
                >
