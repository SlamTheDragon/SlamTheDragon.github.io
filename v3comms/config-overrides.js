module.exports = function override(config, env) {
    ...
    config.output.filename = "static/js/commissions.js";
    config.output.chunkFilename = "static/js/commissionschunk.js";
    config.output.path = config.output.path.replace("build", "build");

    // Change the CSS filename and path
    config.plugins.forEach((plugin) => {
        if (plugin.constructor.name === "MiniCssExtractPlugin") {
            plugin.options.filename = "static/css/commissions.css";
        }
    });
